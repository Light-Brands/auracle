# Designer Naming Specification — The Sovereignty Series

## Cover Text Hierarchy for All 8 Volumes

Each cover follows this exact text hierarchy:

```
┌─────────────────────────────────────────┐
│                                         │
│    The Sovereignty Series               │  ← Series name (small)
│    Volume N                             │  ← Volume marker (small)
│                                         │
│                                         │
│    SEE                                  │  ← TITLE (largest, single word)
│                                         │
│    The Truth That Was Hidden            │  ← Subtitle (medium)
│    in Plain Sight                       │
│                                         │
│                                         │
│    Jennifer Brooke Lawless, M.S.        │  ← Author (smallest)
│                                         │
└─────────────────────────────────────────┘
```

---

## Complete Volume Naming

### Volume 1: See

**Title:** SEE
**Subtitle:** *Recognizing Narcissistic Manipulation in Relationships, Family, and Work*
**Series Line:** The Sovereignty Series — Volume 1
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "Every pattern you can name is a pattern that loses power over you."
**Core Insight:** "This wasn't love—this was conditioning."

---

### Volume 2: Heal

**Title:** HEAL
**Subtitle:** *Nervous System Recovery and Attachment Repair After Narcissistic Abuse*
**Series Line:** The Sovereignty Series — Volume 2
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "Secure attachment is not found—it is practiced."
**Core Insight:** "Leaving hurts because your body learned danger as connection."

---

### Volume 3: Stand

**Title:** STAND
**Subtitle:** *Building Boundaries, Internal Authority, and Self-Trust After Trauma*
**Series Line:** The Sovereignty Series — Volume 3
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "I am not small. I am learning to stand without fear."
**Core Insight:** "You are safe to choose without urgency."

---

### Volume 4: Live

**Title:** LIVE
**Subtitle:** *Reclaiming Presence, Intimacy, and Embodied Leadership*
**Series Line:** The Sovereignty Series — Volume 4
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "My presence is erotic when it belongs to me."
**Core Insight:** "Your presence is yours when it belongs to you."

---

### Volume 5: Give

**Title:** GIVE
**Subtitle:** *Conscious Parenting and Breaking Generational Trauma Cycles*
**Series Line:** The Sovereignty Series — Volume 5
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "The chain breaks with you. Not because you're perfect, but because you're aware."
**Core Insight:** "You don't have to be healed to be a good parent. You have to be healing."

---

### Volume 6: Serve

**Title:** SERVE
**Subtitle:** *Sustainable Helping Without Burnout for Trauma-Informed Guides*
**Series Line:** The Sovereignty Series — Volume 6
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "Your healing is your credential. Your boundaries are your offering."
**Core Insight:** "You cannot give what you don't have. Your healing is your offering."

---

### Volume 7: Thrive

**Title:** THRIVE
**Subtitle:** *Financial Recovery, Career Rebuilding, and Prosperity After Abuse*
**Series Line:** The Sovereignty Series — Volume 7
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "Your prosperity is not a betrayal of your healing. It is a fruit of it."
**Core Insight:** "You are allowed to thrive, not just survive."

---

### Volume 8: Become

**Title:** BECOME
**Subtitle:** *Integration, Identity, and Stepping Into Your Whole Self*
**Series Line:** The Sovereignty Series — Volume 8
**Author:** Jennifer Brooke Lawless, M.S.
**Tagline (back cover):** "You were never becoming someone new. You were always unveiling who you'd been all along."
**Core Insight:** "You were never becoming—you were always unveiling."

---

## The Two Through-Lines (When Displayed Together)

### The Titles (The Mantra)

```
SEE → HEAL → STAND → LIVE → GIVE → SERVE → THRIVE → BECOME
```

Read aloud, these form a single imperative command sequence — a mantra from the reader's truest self.

### The Subtitles (The Poem)

Read together as one continuous thought:

> *The truth that was hidden in plain sight...*
> *The body that remembers the way home...*
> *The ground that was always yours...*
> *The presence that changes everything...*
> *The chain that ends with you...*
> *The light that doesn't consume...*
> *The life you were told you couldn't have...*
> *The self that was never lost.*

### The Noun Progression

The subtitle nouns trace the full healing journey:

```
Truth → Body → Ground → Presence → Chain → Light → Life → Self
```

From **external recognition** (truth) through **somatic healing** (body) to **reclaimed territory** (ground) to **embodied identity** (presence) through **generational break** (chain) to **sustainable purpose** (light) to **material rebuilding** (life) to **total integration** (self).

---

## Design Notes

### Title Pattern

- All 8 volumes use a **single imperative word** as the title
- Each title is a command from the reader's truest self
- Titles form a mantra when read in sequence: *See. Heal. Stand. Live. Give. Serve. Thrive. Become.*

### Subtitle Pattern

- Each subtitle follows the structure: **"The [noun] that [powerful clause]"**
- Read together, they form a single continuous poem
- The subtitles provide the emotional context; the titles provide the directive

### Typography Hierarchy

1. **TITLE** — Largest, most prominent. ALL CAPS. White. Single word — maximum visual impact.
2. **Subtitle** — Medium size, italic or lighter weight. Off-white / warm white. Mixed case. "The [noun] that [clause]" structure.
3. **Series/Volume line** — Small, off-white / warm white, consistent across all 8 covers.
4. **Author name** — Smallest but clearly legible. Off-white / warm white. Consider ALL CAPS.
5. **Accent details** — Subtle gold. Thin rules, dividers, or small decorative marks that add warmth and premium feel.

### Spine Text (Paperback)

```
SEE  |  Recognizing Narcissistic Manipulation in Relationships, Family, and Work  |  Jennifer Brooke Lawless, M.S.
```

When all 8 spines are lined up, the titles read:

```
SEE | HEAL | STAND | LIVE | GIVE | SERVE | THRIVE | BECOME
```

### Color Progression Suggestion

The series should move from darker tones (recognition, crisis) to lighter/warmer tones (healing, embodiment) to radiant tones (sovereignty, transcendence):

| Vol | Title | Background Color Direction |
|:---:|:-----:|---------------------------|
| 1 | SEE | Deep blue — clarity, revelation |
| 2 | HEAL | Warm earthy green — growth, grounding |
| 3 | STAND | Designer's choice — steady, centered |
| 4 | LIVE | Dark pink — vitality, embodiment |
| 5 | GIVE | Warm brown — nurturing, earth |
| 6 | SERVE | Soft green — renewal, purpose |
| 7 | THRIVE | Rich green — abundance, expansion |
| 8 | BECOME | White — transcendence, arrival |

**Title and subtitle text:** White across all 8 volumes for maximum contrast and series cohesion.

### Series Cohesion Requirements

- Same font family across all 8 covers
- **All 8 titles must be the same size** — equal visual weight regardless of character count
- White titles and subtitles across all volumes
- Identical layout structure (title position, author placement)
- "The Sovereignty Series" subtitle on each
- Volume number in consistent position
- Single-word title dominates the cover — this is the brand

---

## Companion Workbook Covers

Each volume has an accompanying **companion workbook**. The workbook covers should be **identical to the main volume covers** — same background color, same white title and subtitle, same layout — with a "Companion Workbook" designation added below the subtitle.

The workbook designation should be:
- White text, lighter weight or smaller size than the subtitle
- Consider decorative rules above and below: `── Companion Workbook ──`
- Clear enough to distinguish from the main volume, subtle enough to still read as part of the set
- When the main volume and its workbook are shelved side by side, they should look like a matched pair

### Workbook Spine Text

```
SEE  |  Companion Workbook  |  Jennifer Brooke Lawless, M.S.
```

---

## File Naming Convention

```
# Individual Volumes
vol1-see-cover-kdp-ebook-2560x1600.jpg
vol1-see-cover-kdp-print-6x9.pdf
vol2-heal-cover-kdp-ebook-2560x1600.jpg
vol3-stand-cover-kdp-ebook-2560x1600.jpg
vol4-live-cover-kdp-ebook-2560x1600.jpg
vol5-give-cover-kdp-ebook-2560x1600.jpg
vol6-serve-cover-kdp-ebook-2560x1600.jpg
vol7-thrive-cover-kdp-ebook-2560x1600.jpg
vol8-become-cover-kdp-ebook-2560x1600.jpg

# Bundles & Mockups
bundle-mockup-complete-series.png
series-spines-lineup.png
```

---

## Series Tagline Options

1. *"From survival to sovereignty — one truth at a time."*
2. *"You were adapting. Now you are choosing."*
3. *"Eight words. Eight books. One unveiling."*
4. *"See. Heal. Stand. Live. Give. Serve. Thrive. Become."*

---

*This document is the definitive naming spec for cover design across all 8 volumes.*
