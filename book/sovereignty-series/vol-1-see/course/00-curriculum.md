# Course 1: You Were Never Crazy

## See: The Truth That Was Hidden in Plain Sight — A Course in Pattern Recognition

---

## Course Overview

**Stage:** SEE
**Duration:** ~11 hours
**Lessons:** 31 video lessons across 6 modules
**Core Insight:** *"This wasn't love—this was conditioning."*

This course teaches you to recognize manipulation patterns in real-time. You will learn to decode 60 manipulation patterns, understand why your nervous system got hooked, and reclaim your perception. By the end, you will trust yourself again.

---

## Learning Outcomes

By completing this course, you will:

1. **Recognize** — Identify manipulation tactics as they happen, not days later
2. **Understand** — Know why these patterns hook your nervous system
3. **Decode** — Use practical tools to see through control disguised as care
4. **Respond** — React from power rather than automatic reaction
5. **Recover** — Trust your own perception again
6. **Reclaim** — Take back your sovereignty and right to peace

---

## Who This Course Is For

- Anyone who has left interactions feeling worse about themselves without understanding why
- Those wondering if they're "too sensitive" or if something really is wrong
- People actively recovering from narcissistic abuse in any relationship
- Those beginning to see family dynamics, friendships, or work relationships with new eyes
- Anyone who wants to recognize patterns before getting pulled in
- Therapists, coaches, and healers who support survivors

---

## Course Modules

### Module 1: Recognition
**Lessons:** 5 | **Duration:** ~1.5 hours

The foundation. Learn to recognize control in your body before your mind catches up. Understand the invisible architecture of manipulation.

| Lesson | Title | Duration | Script |
|--------|-------|----------|--------|
| 1.1 | Welcome to Your Decoder Training | 15 min | [View Script](./lessons/1-1-welcome.md) |
| 1.2 | The Invisible Architecture of Control | 20 min | [View Script](./lessons/1-2-invisible-architecture.md) |
| 1.3 | Your Body Already Knows | 25 min | [View Script](./lessons/1-3-body-knows.md) |
| 1.4 | The 3-3-3 Rule: Your First Tool | 15 min | [View Script](./lessons/1-4-three-rule.md) |
| 1.5 | The Bridge: Preparing for the Playbook | 10 min | [View Script](./lessons/1-5-bridge.md) |

---

### Module 2: The Playbook
**Lessons:** 6 | **Duration:** ~2.5 hours

The tactics. Decode 60 manipulation patterns organized by function. Name what was done to you.

| Lesson | Title | Duration | Script |
|--------|-------|----------|--------|
| 2.1 | How Control Operates | 20 min | [View Script](./lessons/2-1-how-control-operates.md) |
| 2.2 | Information & Perception Tactics | 25 min | [View Script](./lessons/2-2-information-tactics.md) |
| 2.3 | Emotional & Relational Tactics | 25 min | [View Script](./lessons/2-3-emotional-tactics.md) |
| 2.4 | Punishment & Control Tactics | 25 min | [View Script](./lessons/2-4-punishment-tactics.md) |
| 2.5 | Identity & Isolation Tactics | 25 min | [View Script](./lessons/2-5-identity-tactics.md) |
| 2.6 | Why These Tactics Work | 20 min | [View Script](./lessons/2-6-why-tactics-work.md) |

---

### Module 3: The Systems
**Lessons:** 6 | **Duration:** ~2.25 hours

The contexts. See how manipulation operates across family systems, romantic relationships, friendships, and work environments.

| Lesson | Title | Duration | Script |
|--------|-------|----------|--------|
| 3.1a | Trauma Bonds: The Biology of Attachment | 15 min | [View Script](./lessons/3-1a-biology-attachment.md) |
| 3.1b | Trauma Bonds: The Trauma Bond Cycle | 15 min | [View Script](./lessons/3-1b-trauma-bond-cycle.md) |
| 3.2 | Family Roles and Triangulation | 25 min | [View Script](./lessons/3-2-family-roles.md) |
| 3.3 | When Your Parent Is the Narcissist | 25 min | [View Script](./lessons/3-3-parental-wounds.md) |
| 3.4 | Romantic Manipulation Patterns | 25 min | [View Script](./lessons/3-4-romantic-manipulation.md) |
| 3.5 | Workplace, Social, Digital, and Spiritual Contexts | 30 min | [View Script](./lessons/3-5-workplace-social.md) |

---

### Module 4: The Decoder Cards
**Lessons:** 5 | **Duration:** ~2 hours

The tools. Master 60 decoder cards for real-time pattern recognition. Learn the protocol for using them in the moment.

| Lesson | Title | Duration | Script |
|--------|-------|----------|--------|
| 4.1 | Introduction to Decoder Cards | 15 min | [View Script](./lessons/4-1-decoder-intro.md) |
| 4.2 | Core Decoder Cards (Part 1) | 30 min | [View Script](./lessons/4-2-core-cards-1.md) |
| 4.3 | Core Decoder Cards (Part 2) | 30 min | [View Script](./lessons/4-3-core-cards-2.md) |
| 4.4 | Advanced Decoder Cards | 30 min | [View Script](./lessons/4-4-advanced-cards.md) |
| 4.5 | The Decoder Protocol | 20 min | [View Script](./lessons/4-5-decoder-protocol.md) |

---

### Module 5: Response & Recovery
**Lessons:** 5 | **Duration:** ~1.75 hours

The path forward. Rewire your nervous system. Learn what to say. Begin your recovery with grounded next steps.

| Lesson | Title | Duration | Script |
|--------|-------|----------|--------|
| 5.1 | Energetic Remapping | 25 min | [View Script](./lessons/5-1-energetic-remapping.md) |
| 5.2 | Practical Responses and Scripts | 25 min | [View Script](./lessons/5-2-practical-responses.md) |
| 5.3 | The Exposure Questions | 20 min | [View Script](./lessons/5-3-exposure-questions.md) |
| 5.4 | Moving Forward | 15 min | [View Script](./lessons/5-4-moving-forward.md) |
| 5.5 | Course Completion: Your Next Chapter | 15 min | [View Script](./lessons/5-5-course-completion.md) |

---

### Module 6: Advanced Recognition
**Lessons:** 4 | **Duration:** ~1.5 hours

You've come so far. Now let's go deeper. Master advanced patterns, complex dynamics, and the subtle tactics that require trained eyes to spot.

| Lesson | Title | Duration | Script |
|--------|-------|----------|--------|
| 6.1 | Introduction to Advanced Recognition | 15 min | [View Script](./lessons/6-1-advanced-intro.md) |
| 6.2 | Complex Pattern Combinations | 25 min | [View Script](./lessons/6-2-pattern-combinations.md) |
| 6.3 | Digital and Modern Manipulation | 25 min | [View Script](./lessons/6-3-digital-manipulation.md) |
| 6.4 | Mastery and Maintenance | 20 min | [View Script](./lessons/6-4-mastery.md) |

---

## Course Materials

### Included Downloads
- Course Workbook (PDF)
- 60 Decoder Cards (Printable)
- The 3-3-3 Rule Quick Reference
- Practical Response Scripts
- The Exposure Questions Card
- Module Reflection Journals

### Recommended Companions
- Physical copy of *See* (Volume 1)
- Personal journal for integration exercises

---

## How to Take This Course

### Recommended Pace
- 2-3 lessons per week
- Complete each module before moving to the next
- Allow 1-2 days between heavy lessons for integration

### For Each Lesson
1. Find a quiet, private space
2. Have your journal ready
3. Watch without multitasking
4. Complete the exercises before moving on
5. Practice in real life before the next lesson

### Self-Care Protocol
This material may activate memories and emotions. This is normal and part of the healing process. If overwhelmed:
- Pause the lesson
- Use the 3-3-3 Rule
- Ground with the exercises in Lesson 5.1
- Return when ready

---

## What's Next

After completing Course 1, you are ready for:

**Course 2: Heal** — Learn to retrain your nervous system for secure attachment. Understand why insight alone isn't enough. Build the bridge from trauma to trust.

---

## Quick Reference

**The 3-3-3 Rule**
- 3 Seconds: Pause before responding
- 3 Breaths: Inhale 4, exhale 6
- 3 Words or fewer: "I'll think about that"

**The Master Question**
> "When was the last time you were genuinely wrong, admitted it without being forced to, and changed your behavior as a result?"

**The 72-Hour Test**
Wait 72 hours before major decisions about someone. Love-bombing neurochemistry needs time to settle.

---

*"The truth will set you free, but first it will piss you off."*
— commonly attributed to Gloria Steinem
